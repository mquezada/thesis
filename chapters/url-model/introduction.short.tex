...
society.
%
This is particularly true in the face of high impact and breaking news, like
natural disasters and crisis
situations~\cite{ICWSM1817816,castillo2011information}.
...
%
In this way, social media platforms aim to optimize for certain criteria, such
as engagement or views.
%
However, if the user is seeking information about a specific event, looking for
complete and trustworthy information, the aforementioned approaches can result
also in unnecessary exposure to redundant, irrelevant, or even misleading
facts.
%
In this context, delivery of key pieces of information to the user is important,
and even critical in some cases; 
%
for instance, social media is many times used as a complementary source of
information for disaster management and
response~\cite{ICWSM1817816,Sarmiento:2018:DDE:3201064.3201077,Mendoza2019,castillo2011information,Sakaki2010}
%\bp{agregar citas}
and also to obtain information about newsworthy events not yet reported by
formal news outlets, such as breaking news. 
%
...
%
These challenges include, among many others, assessing correctly information
...
%
In particular, we study Twitter, a popular microblog platform, in which users
can post text-based messages.
%
...
The URLs shared in Twitter link to varied types of media (articles,
images, videos, or other tweets), which have been found useful for identifying conversation
topics~\cite{mishne2012twanchor}.
%
In the context of news events, users tend to quickly re-share information if the
event sparks high interest~\cite{kalyanam2016prediction}, which results in many
...
%Dealing with content of these characteristics requires different approaches than with traditional media~\cite{Alonso:2017:WHH:3091478.3091484}.\bp{decir algo sobre a que cosa te refieres con la frase anterior, quizas cambiarlo por algo como: }

%%

In this chapter we focus on the problem of producing summarized representations
of social media data related to news events without significant loss of
information.
%
...
in an event, and to identify relevant content, while significantly reducing the
volume of data.
%
This can be specially useful for online data processing about developing news
and news information seeking tasks.
%
To achieve this, our model annotates shared URLs with the text of the messages
in which they appear.
%
This is, we use messages as {\em anchor text} or {\em surrogate text}. 
%
Firstly, we leverage this idea by identifying relevant URLs that are shared in
social media during a news event.
%
We discard URLs that are too general with respect to an event (e.g. a general
report), or generic (e.g. linking to a news outlet) as we deemed them as
non-relevant.
%
Then we aggregate all the anchor texts associated with relevant URLs into {\em
documents}, and all the conversations around those documents as well, in the
form of retweets and replies.
...
%
We were also able to identify sub-topics by mapping the documents into vectors using dense
word representations, such as neural network--based word embeddings. 
%
We show that the sub-topic detection task yields similar external clustering
criteria measures for our representation, compared to the raw tweets, with
reduced running time.
%
In summary, our contributions are the following:

\begin{enumerate}
  \item We present a novel compact representation for information about news
        events in microblogs, based on shared URLs. This representation reduces
        data volume.

  \item We study the usefulness of our representation through three case studies. We do so by introducing a methodology to represent news events in Twitter and find sub-topics.
%
We show that our methodology displays comparable performance to baseline methods at a fraction of the computational resources.
\end{enumerate}
