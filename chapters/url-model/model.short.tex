...
Our model is based on the assumption that {\em most of the social media posts that discuss a specific event and at the same time contain a URL, are messages that cover a particular portion or sub-topic of the event}.
%
...
\begin{figure}[t]
  ...
  \caption{
    ...
    Two messages are joined by an edge if one shares or replies to the other. 
    A message $m$ and a URL $u$ are joined by an edge if the $m$ shares $u$.
    Each connected component is a {\em document}.
    %A {\em document} is a connected component of the graph. 
  }
...
\subsection{Generation methodology}\label{sec:methodology}
