...
%
%We collected tweets using the Twitter Search API using selected news accounts as seeds for extracting newsworthy search terms.
%
%Given a set of verified news accounts in Twitter (such
%as BBCNews, CNN, Al Jazeera, etc.), we identify the most common keywords across
%their tweets published every hour, and then we use those keywords as search
%terms in the Twitter API.
%\footnote{The full list of news accounts is listed in \url{https://github.com/mquezada/twitter-event-detection/blob/master/settings.py}}
...
% %
% This is possible due that the selected accounts generally employ formal language
% in their tweets. 
% %
...
% Twitter Search API using the top 3 ranked terms for the next hour until the next
% batch of keywords is extracted. 
% The source code of the data collection methodology is
% available\footnote{\url{https://github.com/mquezada/twitter-event- detection/}}.
%
